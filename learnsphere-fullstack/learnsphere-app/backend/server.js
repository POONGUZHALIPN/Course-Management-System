...
// The React app lives in ../frontend and is built with `npm run build`
// there, producing ../frontend/dist. In dev, use the Vite dev server
// (npm run dev in frontend/) instead — this static serving is only used
// for a single-server production run after building.
...
LearnSphere API is running on this port, but no built frontend was found
at frontend/dist.

To use one single server for everything:
  cd frontend
  npm install
  npm run build
  (then restart this backend)

For day-to-day development, it's usually faster to run the frontend's own
dev server instead — keep this backend running, and in another terminal:
  cd frontend
  npm install
  npm run dev
...
function serveStatic(req, res, pathname) {
    ...
    // Prevent path traversal outside the frontend/dist folder
    ...
    fs.stat(filePath, (err, stats) => {
        if (err) {
            if (!fs.existsSync(PUBLIC_DIR)) {
                ...
                return res.end(NO_BUILD_MESSAGE);
            }
            // No file at this path. Real static assets (.css, .js, .png,
            // etc.) that are genuinely missing should still 404 — but
            // anything else, including extensionless client routes like
            // /student-dashboard AND this app's .html alias routes like
            // /login.html (used by window.location.href navigation),
            // should fall back to index.html so React Router can render
            // the right page.
            const ext = path.extname(pathname).toLowerCase();
            const isRealAsset = ext && ext !== '.html' && MIME_TYPES[ext];
            if (!isRealAsset) {
                return fs.readFile(path.join(PUBLIC_DIR, 'index.html'), (fallbackErr, content) => {
                    if (fallbackErr) {
